...
RUN pip3 install docker nose parameterized gitpython

# Patch the deprecated function to always fail rather than warn
RUN sed -i '/^    def deprecated/a\ \ \ \ \ \ \ \ removed = True' /usr/local/lib/python3.8/dist-packages/ansible/utils/display.py

# Override CMD
CMD /bin/startup.sh; nosetests -vs /tests
