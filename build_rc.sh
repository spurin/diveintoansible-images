...
while [[ $# -gt 0 ]]; do
   case $1 in
   -l | --local)
      docker build -t spurin/diveintoansible-rc:$(git branch | grep '*' | awk {'print $2'}) .
      exit
      ;;
   -c | --crossbuild)
      ...
      docker buildx build --platform linux/amd64,linux/arm64/v8 -t spurin/diveintoansible-rc:$(git branch | grep '*' | awk {'print $2'}) . --push
      exit
      ;;
   -cr | --crossbuildremote)
